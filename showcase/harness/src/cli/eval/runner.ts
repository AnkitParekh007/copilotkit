...
export interface SlugResult {
  ...
  tests: Record<string, { status: string; duration_ms: number; error?: string }>;
  duration_ms: number;
}
 ...
 */
export function loadTiers(tiersPath: string, allSlugs: string[]): ResolvedTier[] {
...
function tryParsePlaywrightJson(stdout: string): ParsedPlaywright | null {
  try {
    ...
    const tests: Record<
      string,
      { status: string; duration_ms: number; error?: string }
    > = {};
